...
// AP schedules are intentionally isolated from Property Management schedules
// and from AP prepaid schedules, which use backend/uploads/ap-prepaid-schedules.
const uploadRoot = process.env.AP_SCHEDULE_UPLOAD_DIR
    || (process.env.UPLOAD_FOLDER
        ? path.resolve(__dirname, '..', process.env.UPLOAD_FOLDER)
        : path.join(__dirname, '..', 'uploads'));
const EXPORT_DIR = path.join(uploadRoot, 'ap-schedules');
...
function safeFilePart(value, fallback = 'ap-schedule') {
...
async function saveApScheduleWorkbook({ schedule, data }) {
    ...
    try {
        ...
        console.warn(
            `AP schedule workbook could not be written to ${exportPath}; using in-memory download buffer instead:`,
            error.message
        );
...
function deleteSavedApScheduleWorkbook(schedule) {
    const exportPath = getScheduleExportPath(schedule);
