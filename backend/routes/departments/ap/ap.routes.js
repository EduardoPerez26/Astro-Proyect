...
function sendWorkbookDownload(res, workbook, downloadName) {
    if (workbook?.buffer) {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${downloadName || workbook.filename || 'ap-schedule.xlsx'}"`);
...
router.put('/schedules/:id/rename', ...access('accountsPayable', 'editar'), async (req, res) => {
    try {
        ...
        const savedWorkbook = await persistApScheduleWorkbook(req.params.id);
...
router.post('/schedules/:id/duplicate', ...access('accountsPayable', 'crear'), async (req, res) => {
    try {
        ...
        const savedWorkbook = await persistApScheduleWorkbook(result.insertId);
...
router.get('/schedules/:id/export', ...access('accountsPayable', 'exportar'), async (req, res) => {
    try {
        ...
        const downloadName = `${String(schedule.nombre || 'ap-schedule')
            .replace(/[^a-z0-9]+/gi, '-')
            .replace(/^-+|-+$/g, '')
            .slice(0, 90) || 'ap-schedule'}.xlsx`;
