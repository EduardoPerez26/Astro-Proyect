(function () {
    ...
    async function downloadSchedule(id, button = null) {
        ...
        try {
            ...
            const response = await fetch(
                `${apiBase}${AP_API}/schedules/${encodeURIComponent(id)}/export`,
    ...
    async function downloadDocument(id, button = null) {
        ...
        try {
            ...
            link.href = url;
            link.download = filename || `ap-document-${id}`;
            document.body.appendChild(link);
            link.click();
            link.remove();
            URL.revokeObjectURL(url);
    ...
    async function fetchDocumentBlob(id) {
        ...
        return {
            blob,
            filename: decodeDispositionFilename(disposition) || `ap-document-${id}`,
    ...
    async function apiFetch(path, options = {}) {
        return authenticatedFetch(`${AP_API}${path}`, options);
    }
    ...
    async function prepaidApiFetch(path, options = {}) {
        return authenticatedFetch(`${AP_PREPAID_API}${path}`, options);
    }
