(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        initProfileTabs();
        initPasswordExperience();
        updateProfileActionBar('profileConfigPanel');
        cargarPerfil();
    ...
    function initProfileTabs() {
        const tabsContainer = document.querySelector('.profile-settings-tabs');
        ...
        function activateTab(selectedTab) {
            ...
            if (tabsContainer) {
                ...
                tabsContainer.dataset.activeIndex = String(selectedIndex);
            }
            ...
            panels.forEach(function (panel) {
                ...
                panel.classList.remove('is-profile-entering');
                panel.classList.toggle('is-active', isActive);
                panel.hidden = !isActive;

                if (isActive) {
                    void panel.offsetWidth;
                    panel.classList.add('is-profile-entering');
                }
            });
            ...
            updateProfileActionBar(targetId);
        }
        ...
        if (tabsContainer && initiallyActiveTab) {
            tabsContainer.style.setProperty(
                '--profile-active-index',
                String(Math.max(0, tabs.indexOf(initiallyActiveTab)))
            );
        }
    ...
    function initPasswordExperience() {
        const inputs = ['currentPassword', 'newPassword', 'confirmPassword']
            .map((id) => document.getElementById(id))
            .filter(Boolean);

        inputs.forEach((input) => {
            input.addEventListener('input', updatePasswordExperience);
        });
    ...
    function updatePasswordExperience() {
        ...
        const strengthBox = document.getElementById('profilePasswordStrength');

        const rules = {
            profileRuleLength: password.length >= 8,
            profileRuleCase: /[a-z]/.test(password) && /[A-Z]/.test(password),
            profileRuleNumber: /\d/.test(password),
            profileRuleSpecial: /[^A-Za-z0-9]/.test(password),
            profileRuleMatch: Boolean(password && confirmation && password === confirmation)
        };
        ...
        if (rules.profileRuleCase) score += 1;
        if (rules.profileRuleNumber) score += 1;
        if (rules.profileRuleSpecial) score += 1;

        const levels = [
            { label: 'Enter a new password', width: 0, level: 'empty' },
            { label: 'Weak', width: 24, level: 'weak' },
            { label: 'Fair', width: 46, level: 'fair' },
            { label: 'Good', width: 68, level: 'good' },
            { label: 'Strong', width: 86, level: 'strong' },
            { label: 'Excellent', width: 100, level: 'excellent' }
        ];

        const result = password ? levels[Math.min(score, 5)] : levels[0];

        if (strengthBar) strengthBar.style.width = `${result.width}%`;
        if (strengthText) strengthText.textContent = result.label;
        if (strengthBox) strengthBox.dataset.level = result.level;

        const matchStatus = document.getElementById('profilePasswordMatchStatus');
        if (matchStatus) {
            if (!confirmation) {
                matchStatus.textContent = 'Confirmation pending';
                matchStatus.dataset.state = 'idle';
            } else if (password === confirmation) {
                matchStatus.textContent = 'Passwords match';
                matchStatus.dataset.state = 'match';
            } else {
                matchStatus.textContent = 'Passwords do not match';
                matchStatus.dataset.state = 'mismatch';
            }
        }
    }

    function updateProfileCompletion(usuario = {}) {
        const values = [
            Boolean(String(usuario.nombre || '').trim()),
            Boolean(String(usuario.email || '').trim()),
            Boolean(String(usuario.username || '').trim()),
            Boolean(obtenerFotoPerfil(usuario)),
        ...
        if (label) {
            label.textContent = percentage === 100
                ? 'Account setup complete'
                : `${values.length - completed} recommended item${values.length - completed === 1 ? '' : 's'} remaining`;
        }
    }

    function updateProfileActionBar(targetId) {
        ...
        const hint = document.getElementById('profileActionHint');

        const isMfa = targetId === 'profileMfaPanel';
        const isPassword = targetId === 'profilePasswordPanel';

        if (actionBar) {
            actionBar.classList.toggle('is-mfa', isMfa);
        }

        if (resetButton) {
            resetButton.hidden = isMfa;
        }

        if (saveButton) {
            saveButton.hidden = isMfa;
            const html = isPassword
            ...
            saveButton.dataset.defaultHtml = html;
        ...
        if (hint) {
            hint.innerHTML = isMfa
                ? '<i class="fa-solid fa-shield-halved" aria-hidden="true"></i>Authenticator changes use their own protected confirmation flow.'
                : isPassword
                    ? '<i class="fa-solid fa-circle-info" aria-hidden="true"></i>Only password fields will be used when they are completed.'
                    : '<i class="fa-solid fa-circle-info" aria-hidden="true"></i>Only changed profile fields will be updated.';
        }
    }
    ...
    function normalizarPerfilUsuario(usuario = {}) {
        ...
        const departamento = normalized.departamento ?? normalized.department;

        if (departamento && typeof departamento === 'object') {
            normalized.departamento = {
                ...departamento,
                nombre: departamento.nombre || departamento.name || departamento.label || departamento.descripcion || 'No department'
            };
        } else if (typeof departamento === 'string') {
            normalized.departamento = { nombre: departamento };
        } else {
            normalized.departamento = { nombre: 'No department' };
        }
    ...
    function renderPerfil(usuario) {
        usuario = normalizarPerfilUsuario(usuario);
        ...
        setText('profileHeroRole', roleText);
        setText('profileHeroDepartment', departmentText);
        updateProfileCompletion(usuario);
    ...
    function renderMfaState(usuario) {
        ...
        const securityTitle = document.getElementById('profileSecurityStatusTitle');
        const securityText = document.getElementById('profileSecurityStatusText');
        const securityDot = document.getElementById('profileSecurityStatusDot');

        if (securityTitle) {
            securityTitle.textContent = enabled ? 'Two-step verification enabled' : 'Password protection active';
        }

        if (securityText) {
            securityText.textContent = enabled
                ? 'Your password and Microsoft Authenticator protect this account.'
                : 'Enable Microsoft Authenticator to add a second sign-in verification step.';
        }

        if (securityDot) {
            securityDot.classList.toggle('is-enabled', enabled);
        }
    }
    ...
    function setSaving(isSaving, button) {
        ...
        button.innerHTML = isSaving
            ...
            : (button.dataset.defaultHtml || '<i class="fa-solid fa-floppy-disk" aria-hidden="true"></i>Save profile');
    }
})();
